...
Runs row-level LoRA SFT only. TRACE-ECG, TargetMargin, legacy LayoutBind,
and grouped triplet training are disabled.
EOF
}
...
if [[ "${SETTING}" == "l1" ]]; then
  ...
  RUN_NAME="${RUN_NAME:-L1_SameGroup_LoRA_SFT_frac${FRAC}}"
else
